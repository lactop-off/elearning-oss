...
// BaseQuestion does NOT include choices — each variant owns its own choices field.
const BaseQuestion = z.object({
  quizId: z.string().min(1),
  body: z.string().min(1, 'Question is required').max(1000, 'Question is too long'),
  points: z.number().int().min(1, 'Points must be ≥ 1').max(100, 'Points must be ≤ 100').default(1),
});
...
const MultiChoiceQuestion = BaseQuestion.extend({
  ...
  .refine((data) => data.correctChoiceIndices.every((i) => i < data.choices.length), {
    message: 'All correct indices must point at actual choices',
    path: ['correctChoiceIndices'],
  })
  .refine((data) => new Set(data.correctChoiceIndices).size === data.correctChoiceIndices.length, {
    message: 'Correct indices must not repeat',
    path: ['correctChoiceIndices'],
  })
  .refine((data) => data.correctChoiceIndices.length < data.choices.length, {
    message:
...
const TextQuestion = BaseQuestion.extend({
  type: z.literal('TEXT'),
  // No choices, no correct answer index — graded manually by instructor.
});
