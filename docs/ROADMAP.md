...
- [x] **フォーム画面の体裁統一** — `instructor/courses/new` だけ中央寄せ・戻りリンク無しだったのを、他 4 フォームと同じ「上揃え + 戻りリンク + Card」レイアウトに揃えた。auth 系は別 route group の意図的な別レイアウトのため対象外。
- [x] **クイズ受験/結果画面のデザイン強化** — 受験: 各設問を Card 化し、配点/設問タイプを Badge 表示、選択肢の hover/focus/checked 状態をテーマトークンで強化、Submit ボタンに Send アイコン。結果: pass/fail 判定をアイコン円付きの大きなバナーに、各設問を Card + ヘッダーに Check/X 円アイコンで正誤を一目で。E2E のセレクタ・テキストは全て温存。
- [x] **アクセシビリティ再確認** — `@axe-core/playwright` を導入し `tests/e2e/a11y.spec.ts` で主要画面 (home/カタログ/詳細/サインイン/サインアップ/講師ダッシュボード/講師フォーム) の WCAG 2.1 AA を自動検証。検出された唯一の違反 (primarySoft バッジが 4.49:1 でわずかに不足) を解消するため `--primary` を oklch L 0.54→0.50 に微調整。`outline-none` の単独使用なし、見出し階層は上記タスクで統一済み。
